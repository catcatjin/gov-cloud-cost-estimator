...
const { calcCosts, calcAdjustedReserve, calcAdjustedTotalLow, calcAdjustedTotalHigh } = require('../js/calculator.js')
...
test('案例1：M tier adjustedReserve 使用 cloudWan=60，與靜態 reserve 不同', () => {
  ...
  // 以使用者輸入 cloudWan 計算
  const adjRes = calcAdjustedReserve(costsM.buildMid, cloudWan, costsM.maintMid, rate)
...
test('案例2：M tier adjustedTotalLow 使用 adjustedReserve，與靜態 totalLow 不同', () => {
  ...
  const adjTotalLow = calcAdjustedTotalLow(costsM.buildLow, cloudWan, costsM.maintLow, adjRes)
...
test('案例3：L tier adjustedReserve，cloudWan=300 等於靜態 cloudMid=(80+500)/2=290 相近', () => {
  ...
  const adjRes = calcAdjustedReserve(costsL.buildMid, cloudWan, costsL.maintMid, rate)
...
test('案例4：同一組 inputs 下，直接套公式的結果應與函式計算結果完全一致', () => {
  ...
  // 模擬「畫面顯示」使用的計算路徑
  const reserveForDisplay = calcAdjustedReserve(costsM.buildMid, cloudWan, costsM.maintMid, rate)
  const totalLowForDisplay = calcAdjustedTotalLow(costsM.buildLow, cloudWan, costsM.maintLow, reserveForDisplay)
  const totalHighForDisplay = calcAdjustedTotalHigh(costsM.buildHigh, cloudWan, costsM.maintHigh, reserveForDisplay)
...
test('案例5：cloudWan=0 時，adjustedReserve=0（rate=0），adjustedTotalLow=buildLow+maintLow', () => {
  ...
  const adjRes = calcAdjustedReserve(costsS.buildMid, cloudWan, costsS.maintMid, rate)
  expect(adjRes).toBe(0)

  // adjustedTotalLow = buildLow + 0 + maintLow + 0 = buildLow + maintLow
  const adjTotalLow = calcAdjustedTotalLow(costsS.buildLow, cloudWan, costsS.maintLow, adjRes)
