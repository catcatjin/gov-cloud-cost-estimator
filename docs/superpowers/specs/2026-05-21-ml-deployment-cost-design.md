...
Q8 從二元「有/無 AI」擴充為五選一，繼續參與規模評分（WEIGHTS.q8）。這樣保留現有 S/M/L/XL 心智模型，同時讓 AI 風險納入規模判斷。
...
const AI_WORKLOAD_TEMPLATES = {
  ...
  fineTune: {
    ...
    cloudItems: [
      ...
      { id: 'finetuneCompute', label: '首次 fine-tune GPU 工時（一次性）',
        type: 'one-time', estimatedNTD: 15000 },
    ],
  ...
  customTraining: {
    ...
    cloudItems: [
      { id: 'mlWorkspace', label: 'Azure ML Workspace', sku: 'ML Workspace', monthlyNTD: 800 },
      { id: 'trainingCompute', label: '首次訓練 GPU 工時（一次性）',
        type: 'one-time', estimatedNTD: 60000 },
  ...
  traditionalML: {
    ...
    cloudItems: [
      { id: 'mlWorkspace', label: 'Azure ML Workspace', sku: 'ML Workspace', monthlyNTD: 800 },
      { id: 'trainingCompute', label: '首次訓練工時（一次性）',
        type: 'one-time', estimatedNTD: 8000 },
    ],
...
```js
const RETRAINING_MAINT = {
  none:     { monthlyNTD: 0,    label: '不重訓' },
  once:     { monthlyNTD: 0,    label: '一次性（已含建置費）' },
...
+ workloadAi（AI_WORKLOAD_TEMPLATES[選項].cloudItems）         ← 新 ML 項目
+ inferenceItems（INFERENCE_ITEMS[推論方式]）
+ retrainingCloud（RETRAINING_MAINT[訓練頻率]，重訓 GPU 工時）
...
其中：
  工程師人數 = min(tier 預設值 + Σ engineerDelta, tier.max * 1.5)  ← 上限為 tier 最大值的 1.5 倍
  期程月數   = min(tier 預設值 + Σ durationDelta, 18)              ← 上限 18 個月
...
維運費 =
  tier 基礎維運人月
+ AI/ML 監控調校人月（有 AI/ML 時 +0.2~0.5 人月）
+ 定期資料更新人月（有 RAG 或重訓時 +0.1~0.3 人月）
```
